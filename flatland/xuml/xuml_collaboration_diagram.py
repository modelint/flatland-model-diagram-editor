"""
xuml_collaboration_diagram.py – Generates a state machine diagram for an xuml model using the Flatland draw engine
"""
...
from flatland.input.collab_parser import CollabParser
...
class XumlCollaborationDiagram:

    def __init__(self, xuml_view_path: Path, flatland_layout_path: Path, diagram_file_path: Path,
                 rebuild: bool, show_grid: bool, nodes_only: bool, no_color: bool):
        """Constructor"""
        self.logger = logging.getLogger(__name__)
        self.xuml_view_path = xuml_view_path
        ...
        self.logger.info("Parsing the view")
        # Parse the view
        try:
            self.view = CollabParser(view_file_path=self.xuml_view_path, debug=False)
        except FlatlandIOException as e:
            sys.exit(e)
        try:
            self.collab_view = self.view.parse()
        ...
        self.logger.info("Drawing the actors")
        self.nodes = self.draw_actors()
    ...
    def draw_actors(self) -> Dict[str, SingleCellNode]:
